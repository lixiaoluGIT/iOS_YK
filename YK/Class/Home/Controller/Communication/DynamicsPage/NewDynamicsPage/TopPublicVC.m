...
- (void)viewDidLoad
{
    ...
    // 评论 + 照片
    _editv = [[UIView alloc] initWithFrame:CGRectMake(15, BarH, ScreenWidth-15*2, 0)];
...
//    [_editv addSubview:_textView];
    ...
    textView.frame = CGRectMake(15, 15, _editv.frame.size.width-30, 100);
...
//    [_textView addSubview:_placeholderLabel];
    ...
    _addPic.frame = CGRectMake(15, CGRectGetMaxY(textView.frame)+15, 70, 70);
    [_addPic setImage:[UIImage imageNamed:@"fabu"] forState:UIControlStateNormal];
    [_addPic addTarget:self action:@selector(addPicEvent) forControlEvents:UIControlEventTouchUpInside];
    [_editv addSubview:_addPic];
    _editv.frame = CGRectMake(15, BarH, ScreenWidth-15*2, CGRectGetMaxY(_addPic.frame)+20);
...
- (void)nineGrid
{
    ...
    {
        UIImageView *imgv = [[UIImageView alloc] initWithFrame:CGRectMake(15+(width+widthSpace)*(i%4), (i/4)*(width+heightSpace) + CGRectGetMaxY(textView.frame)+15, width, width)];
...
//        delete.backgroundColor = [UIColor greenColor];
        [delete setImage:[UIImage imageNamed:@"guanbi"] forState:UIControlStateNormal];
