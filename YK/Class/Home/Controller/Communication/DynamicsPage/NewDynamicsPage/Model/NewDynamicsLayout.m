...
- (void)resetLayout
{
    ...
    _height += kDynamicsNormalPadding;//上边距
    _height += kDynamicsNameHeight;//昵称高度
    _height += kDynamicsNameDetailPadding;//昵称和文字内容间距
    
    [self layoutDetail];
    _height += _detailLayout.textBoundingSize.height;//文本高度
    
    if (_model.shouldShowMoreButton) {//显示全部按钮
        _height += kDynamicsNameDetailPadding;//间距
        _height += kDynamicsMoreLessButtonHeight;//按钮高度
    }
    if (_model.photocollections.count != 0) {//
        [self layoutPicture];
        _height += kDynamicsNameDetailPadding;//间距
        _height += _photoContainerSize.height;//图片容器高度
    }
...
//    if (_model.likeArr.count != 0) {//点赞
//        [self layoutThumb];
//    }
//    if (_model.commentArr.count != 0) {//评论
//        [self layoutComment];
//    }
