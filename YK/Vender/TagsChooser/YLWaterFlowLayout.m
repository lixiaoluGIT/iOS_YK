...
- (void)setupLayout
{
    self.minimumInteritemSpacing = kSuitLength_H(24 / 2);//同一行不同cell间距
    self.minimumLineSpacing = kSuitLength_V(20 / 2);//行间距
    //support set sectionheader and sectionfooter at the same time
    
//    if (self.currentTag == 3) {
//        self.headerReferenceSize = CGSizeMake(CGRectGetWidth(self.collectionView.frame), kSuitLength_V(50 / 2));//设置section header 固定高度，如果需要的话
//    } else {
        self.headerReferenceSize = CGSizeMake(CGRectGetWidth(self.collectionView.frame), 0);//设置section header 固定高度，如果需要的话
//    }
    
    self.footerReferenceSize = CGSizeMake(CGRectGetWidth(self.collectionView.frame), 0);//设置section footer 固定高度，如果需要的话
    self.sectionInset = UIEdgeInsetsMake(0, kSuitLength_H(45 / 2), 0, kSuitLength_H(45 / 2));
    ...
    for(NSInteger section = 0; section < _framesArray.count; section++){
        ...
        if(currentSectionFrames.count == 0){
            ...
            if (self.currentTag == 3) {
                ...
                if ([self headerAttri:[NSIndexPath indexPathForRow:0 inSection:section] withY:y] != nil) {
                    [attributesArray addObject:[self headerAttri:[NSIndexPath indexPathForRow:0 inSection:section] withY:y]];
                }
...
//                    [attributesArray addObject:[self headerAttri:[NSIndexPath indexPathForRow:0 inSection:section + 1] withY:y + self.footerReferenceSize.height]];
                    ...
                    if ([self headerAttri:[NSIndexPath indexPathForRow:0 inSection:section + 1] withY:y + self.footerReferenceSize.height] != nil) {
                        [attributesArray addObject:[self headerAttri:[NSIndexPath indexPathForRow:0 inSection:section + 1] withY:y + self.footerReferenceSize.height]];
                    }
                   ...
                   currentFrame.origin.y <= visibleRect.origin.y + visibleRect.size.height){
                    ...
                    if(row == 0){
                        ...
                        if (self.currentTag == 3) {
                            ...
                            if ([self headerAttri:currentIndexPath withY:y] != nil) {
...
//                                [attributesArray addObject:[self headerAttri:headerIndexPath withY:currentSectionHeight]];
                                if ([self headerAttri:headerIndexPath withY:currentSectionHeight] != nil) {
                                    [attributesArray addObject:[self headerAttri:headerIndexPath withY:currentSectionHeight]];
                                }
                            }
...
- (void)calculateFrames
{
    ...
    self.headerReferenceSize = CGSizeMake(CGRectGetWidth(self.collectionView.frame), kSuitLength_V(100 / 2));//设置section header 固定高度，如果需要的话
